...
def extract_data():
    with open(path, mode = 'r') as inf: #Open csv file for reading.
        ...
        for row in reader:
            ...
            originalRecords.append(singleRecord)
            counter += 1
...
#This function detects the objects that are truely intersected by the query window.
def detect_true_intersection():
    turn = 0 #Query turns
    for query in queries:
        windowCo = [query[0],query[2],query[1],query[3]] #Window coordinates
        windowEdges = [[windowCo[0],windowCo[1]],[windowCo[2],windowCo[1]],[windowCo[0],windowCo[3]],[windowCo[2],windowCo[3]]] #Tuple that contains the coordinates of each query window edge
        windowSides = [[windowEdges[0],windowEdges[1]],[windowEdges[0],windowEdges[2]],[windowEdges[2],windowEdges[3]],[windowEdges[1],windowEdges[3]]] #Tuple that contains the coordinates of each query window side
        truelyIntersepted = [] #Here will be added the objects that are truely intersepted
        for objID in queryObjectResults[turn]: #Scan only the objects whose MBR's are intersepted
            #First simple case: The window x/y axis completely cover up the respective object MBR axis
            if((windowCo[0] <= originalRecords[objID-1][1][0][0] and windowCo[2] >= originalRecords[objID-1][1][1][0]) or\
                (windowCo[1] <= originalRecords[objID-1][1][0][1] and windowCo[3] >= originalRecords[objID-1][1][1][1])):
                if(objID not in truelyIntersepted):
                        truelyIntersepted.append(objID)

            #Second case: Gotta check individually each line segment for intersections
            points = originalRecords[objID-1][2] #Tuple that has all of the line segment points
            for line in windowSides: #For each window side
                for i in range(len(points)-1): #For each line segment point
                    #Calculate t,u using the formulas provided by the assignment
                    tNumerator = (points[i][0] - line[0][0])*(line[0][1] - line[1][1]) - (points[i][1] - line[0][1])*(line[0][0] - line[1][0])
                    tDenominator = (points[i][0] - points[i+1][0])*(line[0][1] - line[1][1]) - (points[i][1] - points[i+1][1])*(line[0][0] - line[1][0])
                    if(tDenominator != 0):
                        t = tNumerator/tDenominator
                    uNumerator = (points[i][0] - line[0][0])*(points[i][1] - points[i+1][1]) - (points[i][1] - line[0][1])*(points[i][0] - points[i+1][0])
                    uDenominator = (points[i][0] - points[i+1][0])*(line[0][1] - line[1][1]) - (points[i][1] - points[i+1][1])*(line[0][0] - line[1][0])
                    if(uDenominator != 0):
                        u = uNumerator/uDenominator
                    if(0 <= t <= 1 and 0 <= u <= 1): #If true there's an intersection
                        if (objID not in truelyIntersepted):
                            truelyIntersepted.append(objID)
        turn += 1
